...
from domino.lib.animation import anime
...
def __mirror_flip_pose(*args):
    anime.mirror_pose(args[0], args[1])
...
def __message_dialog(message, *args, **kwargs):
    from domino.assembler.ui.markdown_view import MarkdownView
...
def __quick_menu(parent_menu, current_control):
    ...
    selected_controller = [x for x in selected if mc.objExists(f"{x}.is_ctl")]

    child_controller = controller.get_children(current_control)
    child_controller.append(current_control)

    connected_sets = mc.listConnections(current_control, type="objectSet") or []
    ...
    hosts = [mc.listConnections(sel + ".component_host",
                                destination=False,
                                source=True)[0] for sel in selected]
    ...
    current_control_root = [x for x in mc.listConnections(f"{current_control}.message",
                                                          destination=True,
                                                          source=False,
                                                          type="transform") if mc.objExists(x + ".component_id")][0]
    ...
    mc.menuItem(parent=parent_menu,
                label="Mirror",
                command=partial(__mirror_flip_pose, [current_control], False))
    mc.menuItem(parent=parent_menu,
                label="Mirror below",
                command=partial(__mirror_flip_pose, child_controller, False))
    mc.menuItem(parent=parent_menu,
                label="Flip",
                command=partial(__mirror_flip_pose, [current_control], True))
    mc.menuItem(parent=parent_menu,
                label="Flip below",
                command=partial(__mirror_flip_pose, child_controller, True))
    mc.menuItem(parent=parent_menu, divider=True)
    ...
    if mc.getAttr(current_control_root + ".component") == "assembly":
        ...
        for pose, data in pose_data.items():
            mc.menuItem(parent=pose_menu, label=pose, command=partial(attribute.apply_attr, data, namespace))
...
def domino_quick_menu(*args, **kwargs):
    ...
    if type(args[1]) != bool:
        ...
        if sel and mc.objExists(f"{sel[0]}.is_ctl"):
